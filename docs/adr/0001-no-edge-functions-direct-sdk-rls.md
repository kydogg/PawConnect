...
The Master Checklist prescribed 13 Supabase Edge Functions for Phase 1 (`signUpWithEmail`, `signIn`, `users:updateRole`, `pets:create`, …), but the shipped auth code already calls the Supabase SDK directly, and none of these operations need server-side authority. We decided the app talks straight to Supabase Auth/Postgres/Storage via supabase-swift, with Row Level Security policies as the guard and a Postgres trigger creating the `users` row on signup — zero Edge Functions in Phase 1.
